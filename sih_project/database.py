...
class WorkspaceData:
    def __init__(self):
        ...
        self.cursor.execute("CREATE TABLE IF NOT EXISTS login(username TEXT PRIMARY KEY, password TEXT, email TEXT, user_type TEXT)")
        self.cursor.execute("CREATE TABLE IF NOT EXISTS hackathons(hack_name TEXT, hack_date DATE ,hack_location TEXT)")
        self.conn.commit()
    ...
    def get_hackathons(self):
        self.cursor.execute(f"SELECT * FROM hackathons")
        data = self.cursor.fetchall()
        res = []
        for i in data:
            l = {}
